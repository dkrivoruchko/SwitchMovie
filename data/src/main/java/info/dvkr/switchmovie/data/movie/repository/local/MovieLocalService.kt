...
class MovieLocalService(
    ...
    private var localMovieList by bindPreference(
        preferences,
        MovieLocal.LocalList.LOCAL_LIST_KEY,
        MovieLocal.LocalList()
    )
    ...
    suspend fun updateMovie(inMovie: Movie): Int = movieMutex.withLock {
        ...
        val localMovie = MovieLocal.LocalMovie(
            inMovie.id,
            inMovie.posterPath,
            inMovie.title,
            inMovie.overview,
